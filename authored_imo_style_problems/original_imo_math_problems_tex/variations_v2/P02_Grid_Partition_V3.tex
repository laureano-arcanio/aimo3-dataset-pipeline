...
\title{\Huge\bfseries Grace and Henry's Grid Partition}
...
Grace and Henry are observing a square grid divided into $5041\times 5041$ unit cells arranged in rows and columns.
Each cell contains either a \emph{star} or a \emph{moon}, arranged in a checkerboard pattern so that any two cells sharing a side contain different symbols.
...
Because the grid is colored in a checkerboard pattern, each step switches the type of symbol encountered.
Thus the symbols along a trail strictly alternate.

If a trail contains an even number of cells, it visits the same number of stars and moons.
...
Grace assigns one trail to each column of the field.
Each trail starts at the top plot of its column and proceeds straight downward through all $5041$ plots in that column.
...
To show that Bob cannot do better, Henry argues as follows.
...
There are exactly $4\cdot 5041$ such entry points.
Each beam reflects off the diagonal mirrors and eventually exits the boundary, thereby pairing the boundary edges into $2\cdot 5041$ disjoint pairs.
...
On the other hand, among the $2\cdot 5041$ boundary pairs, at least $5041$ pairs must connect a horizontal boundary edge with a vertical boundary edge.
...
Bob has exhibited a covering using $5041$ trails, and Marley has shown that no covering can use fewer.
