...
\title{\Huge\bfseries P02 Minimum Balanced Trail Partition}
\author{\Large Laureano Arcanio \quad \large (February 2026)}
\date{}
\maketitle
\section*{Problem}

A square grid of size $2073\times 2073$ is colored in a checkerboard pattern so that any two cells sharing a side have different colors.
...
Each trail starts at the top cell of its column and proceeds straight downward through all $2073$ cells in that column.
...
There are exactly $4\cdot 2073$ such entry points.
Each beam reflects off the diagonal mirrors and eventually exits the boundary, thereby pairing the boundary edges into $2\cdot 2073$ disjoint pairs.
...
On the other hand, among the $2\cdot 2073$ boundary pairs, at least $2073$ pairs must connect a horizontal boundary edge with a vertical boundary edge.
...
A covering using $2073$ trails exists, and the lower bound shows that no covering can use fewer.
