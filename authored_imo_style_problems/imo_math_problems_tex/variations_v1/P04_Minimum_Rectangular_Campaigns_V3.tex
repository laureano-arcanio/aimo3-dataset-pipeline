...
\title{\Huge\bfseries P04 Minimum Rectangular Campaigns V3}
...
L\cdot D \ge n.
\]
Since $n=230^2$, we have $\max(L,D)\ge 230$.
...
\[
\text{number of campaigns} \ge n + (L-1) + (D-1) \ge n + 2\cdot 230 - 3.
\]

Now construct a matching configuration.
Partition the $n\times n$ grid into $230\times230$ blocks of size $230\times230$.
Choose the permutation $\pi$ so that exactly one unscheduled task lies in each block row
and block column.
Tile the interior blocks with $230^2$ rectangular campaigns and use $2\cdot230-3$
...
By Erd\H{o}s--Szekeres, there exists a chain or antichain of size at least $230$.
...
\]
rectangles in total.
The block construction for $n=230^2$ achieves this bound, so it is optimal.
