...
\title{\Huge\bfseries P04 Rectangle Cover Permutation Holes V2}
\author{\Large Laureano Arcanio \quad \large (February 2026)}
\date{}
\maketitle
\section*{Problem}

An operations grid spans $n$ consecutive days and $n$ workstations, both indexed
$1,2,\dots,n$.
...
L\cdot D \ge n.
\]
Since $n=200^2$, we have $\max(L,D)\ge 200$.
...
\[
\text{number of campaigns} \ge n + (L-1) + (D-1) \ge n + 2\cdot 200 - 3.
\]

Now construct a matching configuration.
Partition the $n\times n$ grid into $200\times200$ blocks of size $200\times200$.
Choose the permutation $\pi$ so that exactly one unscheduled task lies in each block row
and block column.
Tile the interior blocks with $200^2$ rectangular campaigns and use $2\cdot200-3$
...
By Erd\H{o}s--Szekeres, there exists a chain or antichain of size at least $200$.
...
\]
rectangles in total.
The block construction for $n=200^2$ achieves this bound, so it is optimal.
